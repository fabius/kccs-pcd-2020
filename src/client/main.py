...
try:
    SHARED_PRIME = 23 # p - for demonstration only! to be replaced by a secret (bigger prime)
    SHARED_BASE  = 5  # g 
    my_secret    = int(os.environ["DH_SECRET"]) # a
except KeyError as key:
    lgging.error(f"Diffie Hellman requires a private secret. {key} is unset! Please export it")
    exit(1)
...
if __name__ == "__main__":
    ...
    for contact_number, contact_last_interaction in address_book:
        ...
        my_combination_hashed    = hashlib.sha1(my_combination    .encode()).hexdigest()
        friend_combination       = contact_number  + MY_PHONE_NUMBER + ':' + contact_last_interaction
        friend_combination_hashed= hashlib.sha1(friend_combination.encode()).hexdigest()
    ...
    # 2) hash + secret: list
    resp = requests.post(f"http://{BASE_URL}/compare/", data=json.dumps(array_to_post))

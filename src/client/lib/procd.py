...
class Procd:
    ...
    def run(self):
        ...
        secrets_to_send = [hash_val +
                           sec for hash_val in my_combinations_array]
        ...
        print(array_to_post)
        requests.post(
            f"http://{self.procd_server}/compare/", data=json.dumps(secrets_to_send))
